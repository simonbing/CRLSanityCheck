...
def generate_artificial_data(num_comp,
    ...
    # Change random seed
    random_seed = random_seed + num_data + num_layer
    ...
    if num_data_test is not None:
        s_te, y_te = gen_source_pcl(num_comp, num_data_test, ar_coef, random_seed=random_seed + 1)  # change random_seed
        # Use same parameters of the training data
        x_te, mixlayer_test = apply_mlp_to_source(s_te, num_layer, random_seed=random_seed)  # use same seed
    else:
        s_te = None
        x_te = None
        y_te = None
